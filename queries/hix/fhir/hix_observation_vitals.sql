-- HIX vital-signs Observation query
-- Vital-sign measurements (blood pressure, heart rate, SpO2, temperature, weight, etc.)
-- are stored in a separate monitoring table in HIX with a different schema.
-- Template vars: .effective_from / .effective_to  (FHIR date param)
...
-- Single fixed category row per vital — no join needed.
SELECT
    vital_id                        AS resource_id,
    vital_id || '_cat'              AS id,
...
-- ── Statement 3: Category coding (LOINC vital-signs panel) ────────────────
SELECT
    vital_id                            AS resource_id,
